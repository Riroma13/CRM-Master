...
## Pending Modifications (from client-platform change — not yet archived)

> The `client-platform` change extends this doorbell along a second isolation axis:
> cross-client isolation within the same tenant. The authoritative delta lives at
> `openspec/changes/client-platform/specs/data-leak-detection/spec.md` and will be
> merged into the Requirements above at archive time. The block below is the
> pending MODIFIED requirement and the new ADDED requirement mirrored here for
> visibility.
