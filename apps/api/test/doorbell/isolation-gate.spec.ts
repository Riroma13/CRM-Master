...
describe('🔔 DOORBELL — Multi-tenant isolation gate', () => {
  ...
  it('MUST NOT leak data between tenants via scoped client', async () => {
    ...
    const leaked = clientesB.find((c: any) => c.nombre === 'Cliente-Secreto-A');
    expect(leaked).toBeUndefined();
    expect(clientesB).toHaveLength(0);
  });

  // ─── Test 2: Raw SQL sin tenant_id ────────────────────────────────

  it('MUST return 0 results when raw SQL bypasses tenant scope', async () => {
    const result: any[] = await prismaAdmin.$queryRawUnsafe(
      'SELECT * FROM clientes WHERE "tenantId" = $1',
      TENANT_A_ID,
    );

    // Si raw SQL devuelve datos, el test doorbell falla
    // La extensión de Prisma debería bloquear estas queries
